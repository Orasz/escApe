...
string room::getTexture(){
	return texture;
}

 void room::setObject(Objectt obj){
  ob = obj;
}

Objectt room::getObject(){
  return ob;
}
//MAP CLASS
...
mappa::mappa(int rawss, int colss, int empty_proba){
  ...
  // scala dimensione mappa a seconda dello schermo
	map_dim[0] = (sf::VideoMode::getDesktopMode().width * (float) MAP_WIDTH_RATIO) / 100;
	map_dim[1] = sf::VideoMode::getDesktopMode().height;
	outlinewidth = 10; // spazio fra le Rooms
	for(int i = 0; i < raws; i++){ //genera le rooms della mappa con probabilità di default che siano vuote
		for(int k = 0; k < cols; k++){
			if (i != 0 and i != raws -1 and k != 0 and k != cols -1)
				grid[i][k] = room(empty_prob);
			else // prima e ultima riga così come prima e ultima colonna la probabilità che siano vuote è 0
				grid[i][k] = room(0);
		}
	}
...
vector<vector<reference_wrapper<room>> > mappa::getGrid(){ // metodo getter per il vettore bidimensionale grid
...
int mappa::getEmptyProb(){
	return empty_prob;
}

int mappa::getOutLineWidth(){
	return outlinewidth;
}

void mappa::setOutLineWidth(int val){
	outlinewidth = val;
}

vector<int> mappa::getRoomDim(){
	return room_dim;
}

vector<vector<sf::RectangleShape> > mappa::generateMapShapes(){
	vector<vector<sf::RectangleShape> > shapes(raws);//genera matrice di oggetti shapes
	sf::Texture tmp;
  //calcola dimensioni quadrati/stanze prendendo quelle della mappa e dividendole per il numero,dinamico, di righe e colonne
  ...
  //genera i quadrati e regola posizione, dimensione,  bordo e colore
	for(int i = 0; i < raws; i++){
		for(int k = 0; k < cols; k++){
			...
			tmp_shape.setPosition(k * (room_dim[0] + outlinewidth) + outlinewidth, i * (room_dim[1] + outlinewidth) + outlinewidth);
			tmp_shape.setOutlineThickness(outlinewidth);
			tmp_shape.setOutlineColor(sf::Color::Transparent);
		  if (grid[i][k].isEmpty()){ // se la stanza è vuota essa non viene riempita dalla texture e presenta l'immagine di background
...
game::game(){
  ...
  objs = 1;
	...
	apesBadTurns = {MAX_TURNS}; //vector utilizzato per inibire il movimento alle scimmie
	pg_cord = {0, 0};
	setApesCord();
	setStairsCord();
  setObjCord();
...
//funzione che prende come parametri una stringa e le sue coordinate e restituisce l'oggetto sf::Text
...
sf::Sprite game::getPgShape(){
	...
	int pg_pos[2];
	sf::Sprite pg;
	pg.setPosition(((room_dim[0] + map.getOutLineWidth()) * pg_cord[1]) + ((room_dim[0] + map.getOutLineWidth()) * 0.25), ((room_dim[1] + map.getOutLineWidth()) * pg_cord[0]) + ((room_dim[1] + map.getOutLineWidth()) * 0.25));
	return pg;
}
...
sf::Sprite game::getStairsSprite(){
	vector<int> room_dim = map.getRoomDim();
	sf::Sprite stairs;
	stairs.setPosition(((room_dim[0] + map.getOutLineWidth()) * stairs_cord[1]) + ((room_dim[0] + map.getOutLineWidth()) * 0.15), ((room_dim[1] + map.getOutLineWidth()) * stairs_cord[0]) + ((room_dim[1] + map.getOutLineWidth()) * 0.15));
	return stairs;
}
...
vector<sf::Sprite> game::getApesSprite(){
	...
	for(int i = 0; i < apes; i++){
		sf::Sprite tmp;
		tmp.setPosition(((room_dim[0] + map.getOutLineWidth()) * apes_cord[i][1]) + ((room_dim[0] + map.getOutLineWidth()) * 0.25), ((room_dim[1] + map.getOutLineWidth()) * apes_cord[i][0]) + ((room_dim[1] + map.getOutLineWidth()) * 0.25));
		sprites.push_back(tmp);
	}
	return sprites;
}
// Restituisce un vettore contenente gli oggetti grafici che rappresentano gli oggetti 'bonus'
vector<sf::Sprite> game::getObjectsSprites(){
	vector<sf::Sprite> sprites;
	vector<int> room_dim = map.getRoomDim();
	for(int i = 0; i < obj_cord.size(); i++){
		sf::Sprite tmp;
		tmp.setPosition(((room_dim[0] + map.getOutLineWidth()) * obj_cord[i][1]) + ((room_dim[0] + map.getOutLineWidth()) * 0.25), ((room_dim[1] + map.getOutLineWidth()) * obj_cord[i][0]) + ((room_dim[1] + map.getOutLineWidth()) * 0.25));
		sprites.push_back(tmp);
	}
	return sprites;
}
...
void game::setApesCord(){
	...
	for(int i = 0; i < apes; i ++){
      ...
      //coordinate non possono essere assegnate se la stanza è vuota o se vi è già un'altra scimmia
 		}while(map.getGrid()[tmp[0]][tmp[1]].get().isEmpty() or find(apes_cord.begin(), apes_cord.end(), tmp) != apes_cord.end());
		apes_cord[i] = tmp;
	}
}
...
void game::moveApes(){
	...
	vector<int> tmp;
	for(int i = 0; i < apes; i ++){
		Direction dir; //direzione viene stabilità calcolando distanza fra scimmie e pg
		tmp = apes_cord[i];
		...
		apes_cord[i] = move(apes_cord[i], dir); // muove ogni scimmia nella direzione precedentemente stabilita
		if (operator==(apes_cord[i], stairs_cord))//se le coordinate della scimmia sono divenute le stesse delle scale la scimmia rimane ferma alla posizione precedente
			apes_cord[i]  = tmp;
		if(operator==(apes_cord[i], tmp) and abs(tmp[0] - pg_cord[0]) <= abs(tmp[1] - pg_cord[1])){
			if(tmp[0] - pg_cord[0] > 0)
				dir = Direction::up;
			else
				dir = Direction::down;
...
//funzione che prende input di movimento da tastiera e gestisce i possibili casi
...
//se pg e oggetto hanno le stesse coordinate quest'ultimo è raccolto
  if (find(obj_cord.begin(), obj_cord.end(), pg_cord) != obj_cord.end() && bag.size() < MAX_BAG){
      pickUp(map.getGrid()[pg_cord[0]][pg_cord[1]].get().getObject());//inserimento oggetto in borsa
			obj_cord.erase(find(obj_cord.begin(), obj_cord.end(), pg_cord));//rimozione oggetto
			map.getGrid()[pg_cord[0]][pg_cord[1]].get().setObject(Objectt::Null);
    }
    //se tasto premuto è 1...5 usa relativo oggetto, che viene poi rimosso dalla borsa
  ...
  //se è un "bad turn" le scimmie non muovono
	else if (find(apesBadTurns.begin(), apesBadTurns.end(), apesMoves) == apesBadTurns.end())
			moveApes();
		else
			apesMoves ++;
// se pg e scale hanno stesse coordinate chiama funzione nextLevel
	if (operator==(pg_cord, stairs_cord))
		nextLevel();
//se scimmie e pg hanno stesse coordinate diminuisci di 1 la vita, se scende a 0 gameover.
...
//funzione chiamata se pg e scale hanno stesse coordinate, rigenera la mappa aumentando di 1 righe,colonne, oggetti e scimmie
void game::nextLevel(){
	pg_cord = {0,0};
	int map_dim[2];
	int empty_prob;
...
//assegna coordinate agli oggetti in modo casuale, in stanze vuote e ove non vi siano scimmmie
void game::setObjCord(){
    ...
    std::vector<Objectt> toPick;//vettore da cui , in modo random, verrà preso l' oggetto
    ...
    map.getGrid()[tmp[0]][tmp[1]].get().setObject(toPick[rand() % toPick.size()]);
...
void game::useObjectt(Objectt obj){
  switch (obj) {
    case(Objectt::leftArrow) :{ // le "Arrow" muovono le scimmie in direzione della freccia
      for(int i = 0; i < apes; i++)
    ...
    case(Objectt::KingsOrder):{//le scimmie muovono,se possibile, verso il centro
      vector<int> tmp;
      for(int i = 0; i < apes; i ++){
        Direction dir;
        tmp = apes_cord[i];
        if(abs(tmp[0] - (map.getRaws()/2)) > abs(tmp[1] - (map.getCols()/2))){
    ...
    case(Objectt::soHeavy):{//le scimmie muovono ogni 2 turni,ovvero quando apesMoves != apesBadTurns
...
vector<int> game::move(vector<int> cord, Direction dir){
	...
	if (dir == Direction::up)
		...
		tmp_cord[1] = cord[1] - 1 ;
//se coordinate andrebbero "fuori" dalla mappa o verso una stanza vuota o verso una scimmia non muove
	if ((tmp_cord[0] >= 0 and tmp_cord[0] < map.getRaws()) and (tmp_cord[1] >= 0 and tmp_cord[1] < map.getCols()) and !map.getGrid()[tmp_cord[0]][tmp_cord[1]].get().isEmpty() and find(apes_cord.begin(), apes_cord.end(), tmp_cord) == apes_cord.end())
		return tmp_cord;
	return cord;
}
...
//Funzione grafica che ha il compito di gestire e mostrare a video tutto cio che la classe game organizza
void game::toPrint(){
  ...
  sf::Sprite guide_sprite;
	...
	vector<int> room_dim = map.getRoomDim();
	vector<string> obj_textures_names = {"graveYard.png", "iceAge.png", "kingsOrder.png", "leftArrow.png", "rightArrow.png", "shuffle-it.png", "soHeavy.png", "straightToTheMoon.png", "topArrow.png", "byTheSkin.png", "downArrow.png"};
	//Texture loading
	//Rooms texture
	sf::Texture tmp_room;
	tmp_room.loadFromFile(grid[0][0].get().getTexture(),  sf::IntRect(0, 0, room_dim[0], room_dim[1]));
	for(int i = 0; i < int(NRAWS * 4); i++){
		for(int k = 0; k < int(NCOLS * 3); k++){
			if (modular_textures){
				sf::Texture tmp;
				if (!tmp.loadFromFile(grid[i][k].get().getTexture(),  sf::IntRect(0, 0, room_dim[0], room_dim[1])))
  ...
  guide_texture.loadFromFile("pics/objectGuide.png");
	...
	background_sprite.setScale(float(window_dim[0])/size.x, float(window_dim[1])/size.y);
	size = sidemenu_sprite.getTexture()->getSize();
	sidemenu_sprite.setScale(float(window_dim[0] * 0.01 * (100 - MAP_WIDTH_RATIO))/size.x, float(window_dim[1])/size.y);
	...
	gameover_sprite.setScale(float(window_dim[0])/size.x, float(window_dim[1])/size.y);
	start_sprite.setTexture(start_texture);
	size = start_sprite.getTexture()->getSize();
	start_sprite.setScale(float(window_dim[0])/size.x, float(window_dim[1])/size.y);
  guide_sprite.setTexture(guide_texture);
  guide_sprite.setScale(float(window_dim[0])/size.x, float(window_dim[1])/size.y);

	//creazione finestra
	sf::RenderWindow window(sf::VideoMode(window_dim[0], window_dim[1]), "escAPE!", sf::Style::Fullscreen);
	while (window.isOpen())
	{
		sf::Event event;
        ...
        break;
			...
			case(GameState::active):{
				...
				level_text = getFormattedText(to_string(level), menu_origin.x +  menu_scale.x * 330, 0 +  menu_scale.y * 370);
				apes_text = getFormattedText(to_string(apes),  menu_origin.x +  menu_scale.x * 300, 0 +  menu_scale.y * 668);
				lifes_text = getFormattedText(to_string(lifes),  menu_origin.x +  menu_scale.x * 330, 0 +  menu_scale.y * 468);
				rooms_text = getFormattedText(to_string(rooms),  menu_origin.x +  menu_scale.x * 330, 0 +  menu_scale.y * 568);
				...
				pg_shape.setScale((float(room_dim[0]) / size.x) * 0.7,  (float(room_dim[1]) / size.y) * 0.7);
				stairs_sprite = getStairsSprite();
				stairs_sprite.setTexture(stairs_texture);
				size = stairs_sprite.getTexture()->getSize();
				stairs_sprite.setScale((float)room_dim[0] / (float)size.x,  (float)room_dim[1] / (float)size.y);
				...
				for(int i = 0; i < apes; i++){
					...
					ape_sprites[i].setScale((float(room_dim[0]) / size.x) * 0.7,  (float(room_dim[1]) / size.y) * 0.7);
					window.draw(ape_sprites[i]);
				}
				for(int i = 0; i < obj_cord.size(); i++){
					obj_sprites[i].setTexture(obj_textures[(int)grid[obj_cord[i][0]][obj_cord[i][1]].get().getObject()]);
					size = obj_sprites[i].getTexture()->getSize();
					obj_sprites[i].setScale((float(room_dim[0]) / size.x) * 0.7,  (float(room_dim[1]) / size.y) * 0.7);
					window.draw(obj_sprites[i]);
				}
				for(int i = 0; i < bag.size(); i++){
					sf::Sprite tmp;
					tmp.setPosition(menu_origin.x +  menu_scale.x * (16 + (i * 120)), 0 +  menu_scale.y * 902);
					tmp.setTexture(obj_textures[(int)bag[i]]);
					size = tmp.getTexture()->getSize();
					tmp.setScale((float(80) / size.x) * menu_scale.x,  (float(80) / size.y) * menu_scale.y);
					window.draw(tmp);
				}
